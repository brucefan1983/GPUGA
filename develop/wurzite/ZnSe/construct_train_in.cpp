...
const float DELTA_E = 0;          // energy shift (eV/atom)

const bool CHANGE_TYPE = true;    // for myself
const int TYPE[4] = {0, 1, 1, 0}; // for myself
...
void get_force(void)
{
    for (int n = 0; n < Nc_force; ++n)
    {
        ...
        for (int na = 0; na < Na[n]; ++na)
        {
            ...
            if (CHANGE_TYPE)
            {
                bool is_type1_0 = (r[2] > 1.5 && r[2] < 4.5);
                bool is_type1_1 = (r[2] > 7.5 && r[2] < 10.5);
                bool is_type1_2 = (r[2] > 13.5 && r[2] < 17);
                if (is_type1_0 || is_type1_1 || is_type1_2) type = 1;
            }
            fprintf
            (
